...
const LrcLine = memo(
  ({ line, lineNum, activeLine, fontSizeKey, alignKey, variant, lineHeightRatio, translationScale, activeScale, inactiveOpacityNear, inactiveOpacityFar, enableBlur, onLayout, onPress }: LrcLineProps) => {
    ...
    // 景深模糊：当前行清晰，非当前行按距离加大模糊（Apple Music 风格，可开关）
    const blurAmount = active ? 0 : distance <= 1 ? 12 : 20

    // 透视倾斜：当前行水平，非当前行按距离倾斜（上方正角度 / 下方负角度，形成扇形透视感）
    const tilt = active ? 0 : distance <= 1 ? 5 : 9
    const tiltDirection = lineNum < activeLine ? 1 : -1
    const rotate = `${tilt * tiltDirection}deg`
...
export default memo((props: LyricViewProps) => {
  const {
    ...
    activeScale = 1.15,
    inactiveOpacityNear = 0.85,
    inactiveOpacityFar = 0.7,
  } = props
